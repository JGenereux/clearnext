 ...
 * @description MVP Command Center for Hackathon Demo.
 * * @hackathon_backend_shortcuts (Keep it Simple!):
 * 1. PERSISTENCE: Use a single 'User' document in the DB to store { oxygen, hunger, happiness }.
 * 2. TASK TOGGLE: Just one PATCH endpoint to flip 'completed' status and increment oxygen.
 * 3. MOOD LOG: Simple POST to a 'Log' collection. No complex analytics needed for the demo.
 * 4. AUTH: Use the existing Login logic to fetch the specific 'Zeynep' profile data on load.
 * * Goal: Ensure that when we refresh during the pitch, Koda isn't reset to 0!
      ...
      <main className="flex-1 p-12 overflow-y-auto relative z-20">
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-12">
          <section className="lg:col-span-2 space-y-8">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative h-64 w-full rounded-[3.5rem] overflow-hidden shadow-2xl border-8 border-white group" >
              <img src={workHubSplash} alt="Forest Splash" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110" />
            ...
            <motion.section initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="bg-white/40 backdrop-blur-xl p-8 rounded-[4rem] border border-white/50 flex flex-col md:flex-row items-center gap-8 relative overflow-hidden" >
              <div className="relative group w-full md:w-64 h-48 flex items-center justify-center bg-emerald-50/50 rounded-[3rem] border-4 border-white shadow-inner overflow-hidden">
                <AnimatePresence mode="wait">
                  <motion.div key={kodaMessage} initial={{ opacity: 0, y: 10, scale: 0.8 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -10, scale: 0.8 }} className="absolute top-4 bg-white px-4 py-2 rounded-2xl shadow-sm border border-emerald-100 text-[11px] font-bold text-emerald-800 text-center z-20" >
                    {kodaMessage}
